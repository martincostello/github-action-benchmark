...
async function loadData(dataPath, isJson = false) {
    try {
        const content = await fs_1.promises.readFile(dataPath, 'utf8');
        const json = isJson ? content : content.slice(exports.SCRIPT_PREFIX.length);
...
async function storeData(dataPath, data, isJson = false) {
    const content = (isJson ? '' : exports.SCRIPT_PREFIX) + JSON.stringify(data, null, 2);
    await fs_1.promises.writeFile(dataPath, content, 'utf8');
    core.debug(`Overwrote ${dataPath} for adding new data`);
}
async function addIndexHtmlIfNeeded(config, additionalGitArguments, dir, baseDir) {
...
async function writeBenchmarkToGitHubPagesWithRetry(bench, config, retry) {
    ...
    const commitMessage = (_c = config.commitMessage) !== null && _c !== void 0 ? _c : `add ${name} (${tool}) benchmark result for ${bench.commit.id}`;
