...
async function cmd(config, additionalGitOptions, ...args) {
    var _a, _b, _c;
    ...
    const userArgs = [
        ...
        '-c',
        `user.name=${(_b = config.commitUserName) !== null && _b !== void 0 ? _b : 'github-action-benchmark'}`,
        '-c',
        `user.email=${(_c = config.commitUserEmail) !== null && _c !== void 0 ? _c : 'github@users.noreply.github.com'}`,
        '-c',
...
async function push(config, token, repoUrl, branch, additionalGitOptions = [], ...options) {
...
exports.push = push;
async function pull(config, token, branch, additionalGitOptions = [], ...options) {
...
exports.pull = pull;
async function fetch(config, token, branch, additionalGitOptions = [], ...options) {
...
exports.fetch = fetch;
async function clone(config, token, ghRepository, baseDirectory, additionalGitOptions = [], ...options) {
...
exports.clone = clone;
async function checkout(config, ghRef, additionalGitOptions = [], ...options) {
